...
public float groundPoundSlideBoostMinTime = 0.15f; // optional: prevents insta-end right after impact
    ...
    private void TextStuff()
    {
        ...
        if (OnSlope())
            text_speed.SetText("" + Round(rb.linearVelocity.magnitude, 1) + "/" + Round(moveSpeed, 1));
        else
            text_speed.SetText("" + Round(flatVel.magnitude, 1) + "/" + Round(moveSpeed, 1));
...
//Ground pound
    ...
    private void GroundPoundImpact()
    {
        ...
        {
            Vector3 vv = rb.linearVelocity;
            vv.y = groundPoundBounceVelocity;
            rb.linearVelocity = vv;
        }
        else
        {
